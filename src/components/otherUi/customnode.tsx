...
const WorkflowNode = memo(({
  ...
  // Enable real-time polling with 500ms interval
  const { data: status } = useWatchNodeStatus(id, {
    enabled: true,
    refetchInterval: 500 // Poll every 500ms for real-time updates
  });

  // Map backend status to NodeStatusIndicator expected format
  const mapStatus = (backendStatus?: string): "loading" | "success" | "failed" | "initial" => {
    if (!backendStatus) return "initial";

    const statusLower = backendStatus.toLowerCase();

    // Map executing/pending to loading
    if (statusLower === "executing" || statusLower === "pending") {
      return "loading";
    }

    // Map error/failed to failed
    if (statusLower === "error" || statusLower === "failed") {
      return "failed";
    }

    // Map success/completed to success
  ...
  console.log('Node status:', { nodeId: id, backendStatus: status?.status, mappedStatus: orgstatus })
