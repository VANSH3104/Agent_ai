...
// UPDATED: StatusBorder with dynamic width support
const StatusBorder = ({
  children,
  className,
  padding = 2, // Default padding for border expansion
}: {
  children: ReactNode;
  className?: string;
  padding?: number; // Optional padding to control border expansion
}) => {
  ...
  return (
    <div className="relative">
      <div
        className={cn(
          "absolute inset-0 rounded-[9px] border-2",
          className,
        )}
        style={{
          top: `-${paddingPx}`,
